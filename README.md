...
directly - and owns printing the order summary, so `System.out` calls stay
out of the domain layer.

`PayNestApplication.main()` only wires objects together and asks the service
to print the summary - it has no business or formatting logic of its own.
